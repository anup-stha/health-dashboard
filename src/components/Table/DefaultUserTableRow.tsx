  ...
  return (
    <>
      <tr key={key} className="px-16 text-xl font-medium text-gray-800 lg:text-xl sm:px-0">
        ...
        <td>
          <BooleanTag type="warning" condition={data.isDoctor} trueStatement="Doctor" falseStatement="Patient" />
        </td>
        <td>{data.email}</td>

        <td>
          <BooleanTag type="error" condition={data.isActive} trueStatement="Active" falseStatement="InActive" />
        </td>
...
export const UserCardView = ({ data, key }: any) => {
  return (
    <div key={key} className="hidden w-full px-4 py-6 bg-white rounded-sm sm:block sm:text-base shadow-E200">
